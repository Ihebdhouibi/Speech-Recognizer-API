...
logger = logging.getLogger("ASR Logger")
...
class ASR(sb.Brain):
    ...
    def run(self):
        """
        Main function of our Acoustic Speech Recognizer
        """
        ...
        sb.utils.distributed.run_on_main(
            ...
            kwargs={
                ...
                "save_json_train": self.hparams["train_annotation"],
                "save_json_valid": self.hparams["valid_annotation"],
                "save_json_test": self.hparams["test_annotation"],
            },
        )
        # we can now directly create datasets for training, valid and test
        datasets = self.dataio_prepare(self.hparams)
    ...
    def compute_forward(self, batch, stage):
        ...
        if self.is_ctc_active(stage):
            # output layer for ctc long-probabilities
    ...
    def is_ctc_active(self, stage):
        """
        Checks if ctc is currently active
    ...
    def prepare_features(self, stage, wavs: tuple):
        """
    ...
    def compute_objectives(self, predictions: dict, batch, stage):
        """
        ...
        ------
        loss: torch.Tensor
                A one-elemnt tensor used for backpropagation the gradient.
        """
        ...
        if stage != sb.Stage.TRAIN:
            ...
            # monitor words error rate and character error rated at valid and test time
            self.wer_metric.append(batch.id, predicted_words, target_words)
            self.cer_metric.append(batch.id, predicted_words, target_words)

        return loss

    def on_stage_start(self, stage, epoch: int):
        """
        Gets called at the begining of each epoch.
    ...
    def on_stage_end(self, stage, stage_loss: float, epoch: int):
        """
        ...
        # perform end-ot-iteraction thins, like annealing, logging etc..
        if stage == sb.Stage.VALID:
            # update learning rate
            old_lr, new_lr = self.hparams.lr_annealing(stage_stats["WER"])
            sb.nnet.schedulers.update_learning_rate(self.optimizer, new_lr)

            # the train logger writes a summary to stdout and logfile
            ...
            # Save the current checkpoint and delete the previous checkpoints.
            self.checkpointer.save_and_keep_only(
                meta={"WER": stage_stats["WER"]}, min_keys=["WER"],
            )

        # We also writes the staticts about test data to stdout and to the log file.
        elif stage == sb.Stage.TEST:
            self.hparams.train_logger.log_stats(
    ...
    def dataio_prepare(self, hparams: dict):
        """
        ...
        -------
        hparams: dict
                This dictionary is loaded from the ASR.yaml file, and it includes
                all the hyperparameters needed for dataset construction and loading.
        Returns:
        ------
        datasets: dict
                Dictionary containing "train", "valid" and "test" keys that corresponds to
                the DynamicItemDataset objects.
        """
        # define audio pipeline. In this case we simply read the path contained in the variable
        # wav with the audio reader.
        ...
        # sorting training data with ascending order makes the code much faster
        # because we minimize zero-padding. In most of the cases, this does not harm the performance.
    ...
    def load_pretrained(self, hparams: dict):
        """
